...
const candidates = ['Devtar', 'Azrul', 'Rizal'];
...
.then((accounts) => {
  ...
  .then((result) => {
    ...
    return deployedContract.methods.totalVotesFor(asciiToHex('Devtar')).call();
  })
  .then((votesRama) => {
    console.log('votesDevtar', votesRama);
    return deployedContract.methods.voteForCandidate(asciiToHex('Devtar')).send();
  })
  .then((voteResult) => {
